...
// @desc     Create a post
// @access   Private
router.post('/', [ auth, [ check('title', 'Enter a short interesting title').isLength({ min: 6 }, { max: 50 }), check('text', 'Text is required').not().isEmpty() ] ], async (req, res) => {
...
router.put('/comment/unlike/:id/:comment_id', auth, async (req, res) => {
	try {
		...
		// remove the like
		comment.likes = comment.likes.filter(({ user }) => user.toString() !== req.user.id);
